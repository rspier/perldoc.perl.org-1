...
var pageIndex = {
  ...
  setup: function () {
    if ($('page_index')) {
      ...
      var pageIndexSettings = new Hash.Cookie('pageIndexSettings', {
        duration: 365,
        path: "/"
      });
  ...
  checkPosition: function () {
    ...
    var pageIndexPosition = {
      x: $('page_index').getStyle('left').toInt(),
      y: $('page_index').getStyle('top').toInt()
    };
    ...
    if (pageIndexPosition.x < 0) {
      newX = 0
    }
    if (windowSize.x < (pageIndexPosition.x + pageIndexSize.x)) {
      newX = Math.max(0, windowSize.x - pageIndexSize.x)
    }
    if (pageIndexPosition.y < 0) {
      newY = 0
    }
    if (windowSize.y < (pageIndexPosition.y + pageIndexSize.y)) {
      newY = Math.max(0, windowSize.y - pageIndexSize.y)
    }
  ...
  checkSize: function () {
    ...
    if (windowSize.x < (pageIndexPosition.x + pageIndexSize.x)) {
      newX = windowSize.x - pageIndexPosition.x
    }
    if (windowSize.y < (pageIndexPosition.y + pageIndexSize.y)) {
      newY = windowSize.y - pageIndexPosition.y - pageIndexFooterSize.y - pageIndexHeaderSize.y
    }
